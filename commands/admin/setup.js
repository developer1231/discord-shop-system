...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (data.length == 0) {
      await execute(
        `INSERT INTO guilds (guild_id, bump_channel, admin_channel, review_channel_id, shop_channel_id, request_channel_id, verification_role_id) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [interaction.guild.id, null, null, null, null, null, null]
      );
    }
      ...
      .setDescription(
        "> Here you can see several options regarding the bot setup. Respectively, you can see a checklist of all actions you have taken / still need to configure and what each task involves. **Remember, all tasks need to be completed to be able to use this bot**.\n> Please select one of the buttons below to start.\n\n> ## Tasks" +
          `\n> - [\\${await isCreated(
            ...
            3,
            interaction
          )}.\n> ### Explanation\n> - Click on the ℹ️ button below to view a list of tasks, their explanation and the steps you must take.\n> - To view changes in the setup list, instead of using */setup* again, you can simply click the 🔄 button to refresh the list.`
      )
